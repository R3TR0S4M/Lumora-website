...
function ApplicationGuide({ navigate }: { navigate: (page: Page) => void }) {
  ...
  return <><PageHero label="Your application guide" title={<>A clear way<br />to <em>get started.</em></>} text="You do not need to have every document or decision ready today. Through Lumora’s university connections, students can apply without paying an application fee. Families are welcome to take part too." action="Start on WhatsApp" onAction={() => document.querySelector('#whatsapp-application')?.scrollIntoView({ behavior: 'smooth' })} aside={<div className="photo-art campus-art"><img src={graduation} alt="A graduate celebrating an academic achievement" /><div className="photo-caption">Your next step<br />starts here.</div></div>}>
    ...
    <section className="free-applications-section"><div><Eyebrow light>Free applications</Eyebrow><h2>Apply with no<br /><em>application fee.</em></h2><p>Through Lumora’s university connections, students can submit their applications without paying an application fee. That means you can focus on choosing the right course and preparing a strong application.</p></div><div className="free-applications-points"><article><span>01</span><h3>More room to plan</h3><p>Use your budget for the parts of your study journey that matter most to you.</p></article><article><span>02</span><h3>Guidance without pressure</h3><p>Ask questions, compare your options and take your next step at a pace that feels right.</p></article><article><span>03</span><h3>Clear from the start</h3><p>We will explain the application process and what you need to prepare before you begin.</p></article></div></section>
...
function CourseFinder({ navigate }: { navigate: (page: Page) => void }) {
  ...
  return <main className="quiz-page"><section className="quiz-intro"><Eyebrow>Course finder</Eyebrow><h1>Find a direction<br /><em>that feels like you.</em></h1><p>This quick guide does not choose a university or make a decision for you. It simply gives you a thoughtful starting point for exploring courses in Malaysia.</p></section><section className="quiz-card" aria-labelledby="quiz-heading"><div><span className="quiz-step">About one minute</span><h2 id="quiz-heading">What are you naturally drawn to?</h2></div><label>1. Which subject area sounds most like you?<select value={interest} onChange={(event) => setInterest(event.target.value)}><option value="" disabled>Choose an area</option><option>Technology and problem-solving</option><option>Design and creative ideas</option><option>People, wellbeing and science</option><option>Business, money and leadership</option><option>Buildings, places and how things work</option><option>Society, communication and justice</option></select></label><label>2. How do you prefer to learn?<select value={style} onChange={(event) => setStyle(event.target.value)}><option value="" disabled>Choose a style</option><option>By creating and trying things</option><option>By analysing ideas and information</option><option>By working with people</option><option>By solving real-world problems</option></select></label><label>3. What matters most in your next step?<select value={goal} onChange={(event) => setGoal(event.target.value)}><option value="" disabled>Choose a priority</option><option>Building practical career skills</option><option>Exploring a subject I enjoy</option><option>Keeping future options open</option><option>Making a positive impact</option></select></label><button className="cta" type="button" disabled={!interest || !style || !goal} onClick={getResult}>See my starting point <Arrow /></button>{result && <div className="quiz-result" role="status" aria-live="polite"><small>Your suggested area to explore</small><h3>{result.title}</h3><p>{result.text}</p><Button onClick={() => navigate('/contact')}>Discuss this with Lumora</Button></div>}</section></main>;
}
