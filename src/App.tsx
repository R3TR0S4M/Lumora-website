...
import homeCampus from './assets/home-campus.jpg';
import homeKualaLumpur from './assets/home-kuala-lumpur.jpg';
import homeCampusHeritage from './assets/home-campus-heritage.jpg';
...
function Home({ navigate }: { navigate: (page: Page) => void }) {
  return <>
    <section className="hero">
      ...
      <div className="hero-campus-photo"><img src={homeCampus} alt="Modern university campus building" /><div><span>Malaysia study guide</span><strong>A place to learn,<br />live and grow.</strong></div></div>
    ...
    <section className="home-place"><div className="home-place-copy"><Eyebrow>Malaysia, beyond the classroom</Eyebrow><h2>A study experience<br />with <em>more around it.</em></h2><p>From an energetic city life to spaces made for focused learning, Malaysia gives students room to build confidence both on and beyond campus.</p><Button onClick={() => navigate('/study-in-malaysia')}>Explore study in Malaysia</Button></div><div className="home-place-images"><img className="home-place-city" src={homeKualaLumpur} alt="Kuala Lumpur skyline with the Petronas Towers" /><img className="home-place-heritage" src={homeCampusHeritage} alt="Historic university building" /></div></section>
...
function CourseFinder({ navigate }: { navigate: (page: Page) => void }) {
  ...
  const getResult = () => {
    const suggestions: Record<string, { title: string; text: string }> = {
      'Business, entrepreneurship & leadership': { title: 'Business, management & entrepreneurship', text: 'Explore business administration, international business, marketing, human resources or entrepreneurship.' },
      'Accounting, finance & economics': { title: 'Accounting, finance & economics', text: 'A useful route if you enjoy numbers, decision-making, business strategy and the way markets work.' },
      'Computer science, software & data': { title: 'Computer science, software & data', text: 'Consider computer science, software engineering, cybersecurity, artificial intelligence or data analytics.' },
      'Engineering, technology & innovation': { title: 'Engineering & technology', text: 'Explore mechanical, electrical, civil, chemical or mechatronics engineering and related technology routes.' },
      'Architecture, planning & built environment': { title: 'Architecture & built environment', text: 'A good starting point for architecture, quantity surveying, construction management, interior design or urban planning.' },
      'Health, medicine & life sciences': { title: 'Health & life sciences', text: 'Consider biomedical science, pharmacy, nursing, nutrition, public health or other science-based routes.' },
      'Psychology, society & human behaviour': { title: 'Psychology & social sciences', text: 'Explore psychology, counselling, sociology, social work or other people-centred study areas.' },
      'Law, policy & international relations': { title: 'Law & international relations', text: 'A possible fit for law, public policy, diplomacy, international studies and related analytical pathways.' },
      'Media, marketing & communication': { title: 'Media, marketing & communications', text: 'Explore digital media, public relations, journalism, advertising, content creation or communication studies.' },
      'Creative arts, design & animation': { title: 'Creative arts & design', text: 'Consider graphic design, animation, fashion, film, interior design or other creative practice pathways.' },
      'Hospitality, tourism & events': { title: 'Hospitality, tourism & events', text: 'A practical people-facing area that can include hotel management, tourism, culinary arts and event management.' },
      'Education, languages & childhood studies': { title: 'Education & language studies', text: 'Explore early childhood education, TESL, language studies or education-focused pathways.' },
    };
  ...
  return <main className="quiz-page"><section className="quiz-hero"><div className="quiz-intro"><Eyebrow>Course finder</Eyebrow><h1>Find a direction<br /><em>that feels like you.</em></h1><p>This guided check-in does not choose a university or make a final decision. It gives you a useful starting point before a more personal conversation about courses in Malaysia.</p><span className="quiz-note">Your answers are preliminary. Lumora can still discuss more course, pathway and degree options with you.</span></div><div className="quiz-gallery" aria-label="Students exploring different study directions"><img className="quiz-image-main" src={courseTechnology} alt="Student working on computer programming" /><img className="quiz-image-top" src={courseEngineering} alt="Student working with engineering equipment" /><img className="quiz-image-bottom" src={courseCreative} alt="Student focused on creative work" /></div></section><section className="quiz-card" aria-labelledby="quiz-heading"><div><span className="quiz-step">A more detailed first look</span><h2 id="quiz-heading">Let’s narrow down<br />what could suit you.</h2></div><label><span>1. What level are you considering?</span><div className="select-control"><select value={level} onChange={(event) => setLevel(event.target.value)}><option value="" disabled>Choose a study level</option><option>Foundation or pre-university</option><option>Diploma</option><option>Bachelor's degree</option><option>Master's degree</option><option>PhD or research degree</option><option>Not sure yet</option></select><i aria-hidden="true">⌄</i></div></label><label><span>2. What is your current route?</span><div className="select-control"><select value={route} onChange={(event) => setRoute(event.target.value)}><option value="" disabled>Choose your current stage</option><option>Finishing secondary or high school</option><option>I have IGCSE, A Levels or IB</option><option>I have a diploma or certificate</option><option>I already have a bachelor's degree</option><option>I am transferring from another programme</option><option>I would like guidance on my pathway</option></select><i aria-hidden="true">⌄</i></div></label><label><span>3. Which subject area interests you most?</span><div className="select-control"><select value={interest} onChange={(event) => setInterest(event.target.value)}><option value="" disabled>Choose an area</option><option>Business, entrepreneurship & leadership</option><option>Accounting, finance & economics</option><option>Computer science, software & data</option><option>Engineering, technology & innovation</option><option>Architecture, planning & built environment</option><option>Health, medicine & life sciences</option><option>Psychology, society & human behaviour</option><option>Law, policy & international relations</option><option>Media, marketing & communication</option><option>Creative arts, design & animation</option><option>Hospitality, tourism & events</option><option>Education, languages & childhood studies</option></select><i aria-hidden="true">⌄</i></div></label><label><span>4. How do you like to learn?</span><div className="select-control"><select value={style} onChange={(event) => setStyle(event.target.value)}><option value="" disabled>Choose a learning style</option><option>By building, making and trying things</option><option>By analysing ideas, research and information</option><option>By working with people and communicating</option><option>By solving real-world practical problems</option><option>By mixing theory with hands-on projects</option></select><i aria-hidden="true">⌄</i></div></label><label><span>5. What matters most for your future?</span><div className="select-control"><select value={goal} onChange={(event) => setGoal(event.target.value)}><option value="" disabled>Choose a priority</option><option>Building career-ready practical skills</option><option>Exploring a subject I genuinely enjoy</option><option>Keeping different career options open</option><option>Making a positive impact on people or communities</option><option>Preparing for further study or research</option></select><i aria-hidden="true">⌄</i></div></label><aside className="quiz-card-help"><span>What happens next</span><strong>This is a starting point, not a final answer.</strong><p>Course names, entry requirements and the best pathway can vary. Lumora will talk through your results, interests and plans in more detail.</p></aside><button className="cta" type="button" disabled={!level || !route || !interest || !style || !goal} onClick={getResult}>See my starting point <Arrow /></button>{result && <div className="quiz-result" role="status" aria-live="polite"><small>Your preliminary direction to explore</small><h3>{result.title}</h3><p>{result.text}</p><div className="quiz-result-details"><span>{level}</span><span>{route}</span><span>{goal}</span></div><p className="quiz-result-note">This is an initial guide only. Talk to Lumora and we can explore additional degree choices, entry routes and course options together.</p><Button onClick={() => navigate('/contact')}>Discuss my options with Lumora</Button></div>}</section></main>;
}
